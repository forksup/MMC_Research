...
class _ChainBase(BaseEstimator):
    ...
    def predict_proba(self, x: np.ndarray) -> np.ndarray:
        ...
        # Get powers of the dimensions, then gets the dot product of each
        idx = np.array(idx[::-1])
...
class MTD(_ChainBase):
    ...
    def train(self,
        ...
        :param X_train: NumPy array of shape (n_samples, order)
                  Training data
        ...
        original_X = deepcopy(X_train)
        original_y = deepcopy(y_train)
        ...
        X_train = self._check_and_reshape_input(X_train)
        X_train = np.hstack([X_train, y_train.reshape(-1, 1)])
        self._calculate_dimensions(X_train)
        X_train = self._aggregate_chain(X_train, sample_weight)

        self._create_indexes()

        n_direct = [X_train.reshape(-1,
                                    self._n_dimensions,
                                    self._n_dimensions ** (self.order - i - 1),
                                    self._n_dimensions).sum(0).sum(1) for i in range(self.order)]
        n_direct = np.array(n_direct)

        candidates = Parallel(n_jobs=self.n_jobs)(delayed(MTD._fit_one)(X_train,
                                                                        self._indexes,
                                                                        self.order,
                                                                        self._n_dimensions,
                                                                        self.min_gain,
